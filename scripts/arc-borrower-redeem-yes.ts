...
console.log(`Redeeming ARC borrower YES collateral on chain ${await publicClient.getChainId()}`);
...
const txHash = await outcomeToken.write.redeem([
  loanView.marketId,
  1,
  loanView.borrowerCollateralAmount,
]);
console.log(`redeem YES tx: ${txHash}`);
const receipt = await publicClient.waitForTransactionReceipt({ hash: txHash });
if (receipt.status !== "success") {
  throw new Error(`redeem YES failed: ${txHash}`);
}

console.log("");
console.log("Borrower YES collateral redeemed:");
console.log(`LOAN_ID=${loanId.toString()}`);
console.log(`MARKET_ID=${loanView.marketId}`);
console.log(`REDEEMED_AMOUNT=${loanView.borrowerCollateralAmount.toString()}`);
