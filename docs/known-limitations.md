...
| --- | --- | --- | --- | --- |
| 10 | Real wallet frontend walkthrough | ARC contracts, a demo loan, activation, direct on-chain YES trade, and mock-signer UI walkthrough are verified, but the same path still needs to be exercised from the browser UI with a real ARC wallet. | In-app browser has no injected EVM wallet, so the reviewer UI path uses the centralized mock signer. | Repeat create/fund/activate/trade from the UI with a real ARC wallet; Circle User-Controlled Wallets are the planned recommended retail path. |
| 9 | Production backend deployment | Local PostgreSQL smoke now passes, but the backend is not deployed as a hosted service. | MVP submission focuses on repo, contracts, ARC evidence, and local reproducibility. | Deploy `dev:clob` equivalent with managed PostgreSQL, RPC configuration, operator key management, monitoring, and logs. |
...
| 8 | Circle wallet integration | The live path currently uses injected EVM wallets; Circle User-Controlled Wallets are documented as the recommended retail target but not integrated in the frontend. | The MVP first proves contracts, CLOB, and ARC settlement without coupling matching to a wallet vendor. | Add Circle User-Controlled Wallet frontend onboarding/signing/transaction flow while preserving injected-wallet and market-maker signer support. |
...
| 8 | Wallet demo fallback | Mock signer can produce demo tx hashes, read demo balances through mock `eth_call`, and sign EIP-712 orders. | In-app browser has no injected EVM wallet, but reviewers need a no-wallet UI path. | Keep mock mode as a clearly labelled UI fallback; production-like path should use `docs/wallet-path.md` and `docs/arc-testnet-runbook.md`. |
...
- Mocks are centralized under `mocks/` and disabled by default unless a demo command or env flag enables them.
...
- Wallet paths are separated in `docs/wallet-path.md`: live injected-wallet path, backend executor wallet, planned Circle retail path, market-maker path, and demo mock path.
