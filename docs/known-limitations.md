...
| --- | --- | --- | --- | --- |
| 10 | Real wallet frontend walkthrough | ARC contracts, a demo loan, activation, direct on-chain YES trade, mock UI, and the Circle provider code are verified, but a hosted Social Login path still needs real credentials. | OAuth and Circle application configuration are deployment-specific. | Repeat create/fund/activate/trade from the public UI with a Circle or injected ARC wallet. |
...
- Wallet paths are separated in `docs/wallet-path.md`: injected wallet, Circle retail wallet, backend executor, market maker, and demo mock paths.
